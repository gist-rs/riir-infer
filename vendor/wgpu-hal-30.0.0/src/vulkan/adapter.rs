...
impl super::Adapter {
    ...
    /// riir-ai Issue 994 vendor patch: total device-local memory of this
    /// physical device, summed over every heap flagged
    ...
    /// — the silent-corruption class of riir-ai Issue 994. This accessor is
    /// the Vulkan half of the budget query; see also the DX12 and Metal twins
    /// and riir-gpu's `vram_budget` module.
    pub fn total_video_memory_bytes(&self) -> Option<u64> {
        let props = unsafe { self.instance.raw.get_physical_device_memory_properties(self.raw) };
