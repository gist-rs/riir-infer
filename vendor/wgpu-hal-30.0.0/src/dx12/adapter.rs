...
impl super::Adapter {
    ...
    /// riir-ai Issue 994 vendor patch: total dedicated video memory of this
    /// adapter, from `IDXGIAdapter4::GetDesc2` -> `DedicatedVideoMemory`.
    ///
    /// The CubeCL/wgpu stack exposes no adapter memory budget anywhere upstream
    /// (wgpu 30's `Limits` carry only per-buffer caps; cubecl's
    /// `MemoryDeviceProperties` carries only page size + alignment), so GPU
    /// constructors had no way to refuse a working set larger than the device
    /// — the silent-corruption class of riir-ai Issue 994 (cubecl pool
    /// exhaustion survived by the main thread, self-consistent garbage).
    /// This accessor is the DX12 half of the budget query; see also the Vulkan
    /// and Metal twins and riir-gpu's `vram_budget` module.
    pub fn total_video_memory_bytes(&self) -> Option<u64> {
        let desc = unsafe { self.raw.GetDesc2() }.ok()?;
