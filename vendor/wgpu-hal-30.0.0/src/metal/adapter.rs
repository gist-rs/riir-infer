...
impl super::Adapter {
    ...
    /// riir-ai Issue 994 vendor patch: the device's recommended VRAM working
    /// set (`MTLDevice.recommendedMaxWorkingSetSize`) — on unified-memory
    /// Apple Silicon this is the OS-advised budget (system RAM minus what the
    /// OS wants for itself), which is the honest ceiling for a GPU-resident
    /// working-set check, not total physical RAM.
    ///
    /// The CubeCL/wgpu stack exposes no adapter memory budget anywhere
    /// upstream, so GPU constructors had no way to refuse a working set larger
    /// than the device — the silent-corruption class of riir-ai Issue 994.
