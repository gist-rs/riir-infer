...
impl Buffer {
    ...
    /// [Plan 536 / Issue 663 fork] Expose the raw Metal buffer handle for
    /// zero-copy interop with custom Metal kernels. Mirrors the public
    /// `Texture::raw_handle()` accessor. Upstream keeps this private by
    /// design; the riir-ai fork exposes it for the Metal parity effort.
    pub fn raw_handle(&self) -> &ProtocolObject<dyn MTLBuffer> {
        &self.raw
    }

    /// [Plan 536 / Issue 663 fork] The size of the buffer in bytes.
    pub fn raw_size(&self) -> wgt::BufferAddress {
        self.size
    }
}
