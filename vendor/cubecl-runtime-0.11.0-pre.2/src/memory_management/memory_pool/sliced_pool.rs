...
pub struct SlicedPool {
    ...
    // ── riir-ai Issue 712 patch (local divergence from upstream 0.11.0-pre.2) ──
    /// Consecutive cleanups each page has been observed fully free. Mirrors
    /// `ExclusiveMemoryPool`'s `free_count`: pages observed fully free on
    /// `ALLOC_AFTER_FREE` consecutive (non-explicit) cleanups are returned to
    /// the storage. Without this, freed *slices* stay committed inside their
    /// pages forever (sliced pools released nothing on periodic cleanup), so
    /// long-lived processes (test binaries running many GPU workloads) grow
    /// committed memory monotonically until the driver OOMs.
    free_counts: Vec<u32>,
    /// Scratch twin of `free_counts` for the cleanup renumbering sweep.
    free_counts_tmp: Vec<u32>,
}

/// How many consecutive non-explicit cleanups a page must be observed fully
/// free before it is deallocated (riir-ai Issue 712 patch — same value as
/// `ExclusiveMemoryPool`'s hysteresis).
...
impl MemoryPool for SlicedPool {
    ...
    fn try_reserve(&mut self, size: u64) -> Option<super::ManagedMemoryHandle> {
        for (index, (page, _)) in self.pages.iter_mut().enumerate() {
            page.coalesce();
            if let Some(handle) = page.try_reserve(size) {
                // riir-ai Issue 712 patch: the page is in use again — restart
                // its fully-free hysteresis so it is never deallocated while
                // it (or a sibling slice) is live.
                if let Some(count) = self.free_counts.get_mut(index) {
                    *count = 0;
                }
                return Some(handle);
            }
    ...
    fn cleanup<Storage: crate::storage::ComputeStorage>(
        ...
        // ── riir-ai Issue 712 patch (local divergence from upstream 0.11.0-pre.2) ──
        //
        // Upstream: non-explicit cleanup is a no-op for sliced pools, so pages
        // were only ever released by an explicit `memory_cleanup` — freed
        // slices stayed committed for the rest of the process. That made any
        // process running many sequential GPU workloads (a libtest binary with
        // hundreds of tests) grow committed memory monotonically until the
        // driver OOM'd — measured on the 4090 (24 GB heap) as the Issue 712
        // full-suite cascade.
        //
        // Now: non-explicit (per-flush) cleanup deallocates pages observed
        // FULLY free on `ALLOC_AFTER_FREE` consecutive cleanups — the same
        // hysteresis `ExclusiveMemoryPool` has always used. Working pages
        // (never fully free) are never released; a page freed between batches
        // and re-reserved before the hysteresis elapses is never yanked. Only
        // genuinely-abandoned pages return to the storage.
        if self.pages.is_empty() {
            return;
        }

        self.pages_tmp.clear();
        self.free_counts_tmp.clear();

        for (index, (mut page, id)) in self.pages.drain(..).enumerate() {
            page.coalesce();
            let summary = page.summary(false);

            let fully_free = summary.amount_free == summary.amount_total;
            let free_count = if fully_free {
                self.free_counts.get(index).copied().unwrap_or(0) + 1
            } else {
                0
            };

            let dealloc = if explicit {
                fully_free
            } else {
                fully_free && free_count >= ALLOC_AFTER_FREE
            };
            ...
            if dealloc {
                ...
                self.free_counts_tmp.push(free_count);
            }
        }

        core::mem::swap(&mut self.pages, &mut self.pages_tmp);
        core::mem::swap(&mut self.free_counts, &mut self.free_counts_tmp);
    }
