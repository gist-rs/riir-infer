...
impl<R: Runtime> ComputeClient<R> {
    ...
    /// Batched variant of [`write`](Self::write) — one device submit for N
    /// destinations (riir-ai Issue 727 H7: the write-side mirror of the batched
    /// `read`; the server API already accepts a Vec natively, so N single
    /// `write` calls were N submits where one suffices).
    ///
    /// Non-blocking: the writes are enqueued on this client's current stream.
    pub fn write_many(&self, writes: Vec<(Handle, Bytes)>) {
        if writes.is_empty() {
            return;
        }
        let stream_id = self.stream_id();
        let descriptors = writes
            .into_iter()
            .map(|(handle, data)| {
                let len = data.len();
                (
                    CopyDescriptor::new(handle.binding(), [len].into(), [1].into(), 1),
                    data,
                )
            })
            .collect::<Vec<_>>();
