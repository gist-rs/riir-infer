...
impl LayerArms {
    fn build(config: &Config, seed: u64, a_log_scale: f32) -> Self {
        ...
        let a_log: Vec<f32> = (0..n_v)
            .map(|_| -a_log_scale * (0.9 + 0.1 * rng.f32_unit()))
            .collect();
...
fn run_lockstep(config: &Config, arms: &LayerArms, window: usize, seed: u64) -> Vec<f32> {
    ...
    let mut scratch = [
        DeltaNetLayerScratch::new(config),
        DeltaNetLayerScratch::new(config),
    ];
...
#[cfg_attr(
    debug_assertions,
    ignore = "matvec-heavy certification — run with --release"
)]
