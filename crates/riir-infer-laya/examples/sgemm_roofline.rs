...
fn main() {
    ...
    let msl = format!("{MSL_HEAD}{MSL_NARROW_COPY}{MSL_ROOFLINE}");
    let lib = device
    ...
    for &(m, k, n, label) in CELLS {
        ...
        // ── position-balanced timed rounds ──
        let mut t_narrow: Vec<u128> = Vec::with_capacity(ROUNDS);
        let mut t_roof: Vec<u128> = Vec::with_capacity(ROUNDS);
        for r in 0..ROUNDS {
            let narrow_first = r % 2 == 0;
            for arm in 0..2 {
                let use_narrow = if narrow_first { arm == 0 } else { arm == 1 };
        ...
        println!(
            "cell {m}x{k}x{n}: narrow p50={us_n:>8.1}us ({tf_n:.2} TF/s{}) | roofline p50={us_r:>8.1}us ({tf_r:.2} TF/s) | narrow is {:+.1}% over roofline  # {label}",
            if bit { ", bit-identical" } else { ", matches" },
            (us_n / us_r - 1.0) * 100.0,
        );
        // Keep the buffers alive past the closure borrows.
        std::hint::black_box((&a_buf, &w_buf, &out_buf, &ra_buf, &rb_buf, &rout_buf));
    }
