...
fn test_chain_matches_sequential_all_deltanet() {
    ...
    let mut cache_tree = crate::deltanet::HybridCache::with_layer_types(&config, &layer_types);
...
fn test_branching_matches_per_branch_all_deltanet() {
    ...
    let find_topo = |token: usize| -> usize {
        (0..4)
            .find(|&k| token_ids[topo.topo_order[k]] == token)
            .unwrap()
    };
...
fn assert_caches_match(
    a: &crate::deltanet::HybridCache,
    b: &crate::deltanet::HybridCache,
    tol: f32,
) {
        ...
        .enumerate()
    {
        assert_eq!(
            sa.len(),
            sb.len(),
            "layer {i}: recurrent state len mismatch"
        );
        if sa.is_empty() {
...
fn test_spec_step_deltanet_weaver_cold_start_no_panic() {
    ...
    let (weaver, mut wscratch) = make_zero_weaver(n_embd, vocab, draft_config.draft_lookahead);
    let mut h_dflash_captured = vec![0.0f32; draft_config.draft_lookahead * draft_config.n_embd];
